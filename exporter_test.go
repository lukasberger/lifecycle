	...
	when("#Export", func() {
		when("previous image exists", func() {
			...
			it("creates app layer on Run image", func() {
				h.AssertNil(t, exporter.Export(layersDir, appDir, fakeRunImage, fakeOriginalImage, launcherPath, mirrors))
			...
			it("saves run image metadata to the resulting image", func() {
				...
				v, err := fakeRunImage.Label("io.buildpacks.run-image.metadata")
				h.AssertNil(t, err)
				h.AssertEq(t, v, `{"runImage":{"image":"some/run","mirrors":["registry.example.com/some/run","other.example.com/some/run"]}}`)
			})
			...
			when("previous image metadata is missing buildpack for reused layer", func() {
				...
				it("returns an error", func() {
					h.AssertError(
						t,
						exporter.Export(layersDir, appDir, fakeRunImage, fakeOriginalImage, launcherPath, mirrors),
		...
		when("previous image doesn't exist", func() {
			...
			it("creates app layer on Run image", func() {
				h.AssertNil(t, exporter.Export(layersDir, appDir, fakeRunImage, nonExistingOriginalImage, launcherPath, mirrors))
		...
		when("there is an invalid layer.toml", func() {
			...
			it("returns an error", func() {
				h.AssertError(
					t,
					exporter.Export(layersDir, appDir, fakeRunImage, mockNonExistingOriginalImage, launcherPath, mirrors),
