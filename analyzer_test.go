...
func testAnalyzer(t *testing.T, when spec.G, it spec.S) {
	...
	when("#Analyze", func() {
		...
		when("image exists", func() {
			it.Before(func() {
				metadata := h.MustReadFile(t, filepath.Join("testdata", "analyzer", "app.json"))
			...
			when("cache exists", func() {
				it.Before(func() {
					metadata := h.MustReadFile(t, filepath.Join("testdata", "analyzer", "cache.json"))
					var cacheMetadata cache.Metadata
